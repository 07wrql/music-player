/**
 * @file    flash_if.c
 * @brief   Flash 操作接口实现 — BootLoader 和 APP 共用
 *
 * 注意事项:
 *  1. FLASH 操作期间 CPU 被 stall，关键操作需关全局中断
 *  2. 必须先 erase 再 write (Flash 只能 1→0, erase 把所有位变回 1)
 *  3. 编程按 Word(32-bit) 粒度
 *  4. 擦除以扇区为单位 (F407: 16KB/64KB/128KB 三种扇区大小)
 */
...
int flash_erase_sectors(uint32_t sector_start, uint32_t sector_count)
{
    ...
    status = HAL_FLASHEx_Erase(&erase, &sector_err);
...
/* ================================================================
   CRC32 (Ethernet standard polynomial)
   用于固件完整性校验
   ================================================================ */
static const uint32_t crc32_table[256] = {
    ...
    0x98D220BC,0xEFD5102E,0x71B18589,0x06B6B51F,0x9FBFE4A5,0xE8B8D433,
