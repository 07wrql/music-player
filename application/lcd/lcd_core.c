...
#include "lcd.h"
#include "sd_task.h"   /* sd_sync() / SD_CMD_FONT_READ */
...
{
    lcd_io_cs(plcd->io, 0);  /* CS=LOW 选中LCD, 之后一直保持 */
...
/* ================================================================
   GBK 中文渲染 — 字库从 SD 卡批量读取, 不占 Flash
   ================================================================ */

#define FONT_CHAR_SIZE  32   /* 每个汉字 32 字节 (16×16/8) */

/**
 * @brief  计算 GBK 字符串的像素宽度
 *         中文字符 = 16px, ASCII 字符 = 8px
 */
uint16_t gbk_str_width(const uint8_t *str)
{
    uint16_t w = 0;
    while (*str) {
        if (*str < 0x80) { w += 8;  str++; }
...
/**
 * @brief  批量读取 GBK 字符串中所有汉字的字模 (一次 sd_sync)
 *         font_data: 输出缓冲 (需 >= 汉字数 * 32 字节)
 *         offsets:   每个汉字的字库偏移量
 *         max_chars: 最大汉字数
 * @retval 实际读取的汉字数
 */
static uint8_t font_batch_read(const uint8_t *str,
                               uint8_t *font_data, uint32_t *offsets,
                               uint8_t max_chars)
{
    uint8_t n = 0;
    while (*str && n < max_chars) {
        if (*str >= 0x80) {
            ...
            if (first >= 0xA1 && first <= 0xF7
                ...
                uint8_t qu  = first  - 0xA1;
                uint8_t wei = second - 0xA1;
                offsets[n] = ((uint32_t)qu * 94 + wei) * FONT_CHAR_SIZE;
                n++;
            }
            str += 2;
        } else {
    ...
    for (uint8_t i = 0; i < n; i++) {
        if (sd_sync(SD_CMD_FONT_READ, NULL,
                    font_data + i * FONT_CHAR_SIZE, FONT_CHAR_SIZE,
                    offsets[i]) != FONT_CHAR_SIZE) {
            /* 读失败: 清空该字模 */
            memset(font_data + i * FONT_CHAR_SIZE, 0xFF, FONT_CHAR_SIZE);
        }
    }
    return n;
}
...
/**
 * @brief  渲染一个汉字 (字模数据已在 font_data 中)
 */
static void lcd_show_gbk_char_data(lcd* plcd, uint16_t x, uint16_t y,
                                   const uint8_t *font)
{
    uint16_t fc = plcd->font.front_color;
    uint16_t bc = plcd->font.back_color;

    /* 一次 set_address 覆盖整个 16x16 字符, 然后连续写 256 个像素 */
    lcd_set_address(plcd, x, y, x + 15, y + 15);
    for (int row = 0; row < 16; row++) {
        uint8_t b1 = font[row * 2];
        uint8_t b2 = font[row * 2 + 1];
        for (int col = 0; col < 8; col++)
            lcd_write_halfword(plcd->io, (b1 & (0x80 >> col)) ? fc : bc);
        for (int col = 0; col < 8; col++)
            lcd_write_halfword(plcd->io, (b2 & (0x80 >> col)) ? fc : bc);
    }
}

/**
 * @brief  显示 GBK 编码字符串 (中英混排, 批量读字库)
 *         字节 < 0x80 → ASCII (当前字体, 宽 8px)
 *         字节 >= 0x80 → GB2312 双字节汉字 (宽 16px)
 */
void lcd_show_gbk_string(lcd* plcd, uint16_t x, uint16_t y, const uint8_t *str)
{
    uint8_t  font_buf[16 * FONT_CHAR_SIZE];  /* 最多 16 个汉字 */
    uint32_t offsets[16];
    uint8_t  ch_idx = 0;

    /* 先批量读取所有汉字的字模 */
    font_batch_read(str, font_buf, offsets, 16);

    /* 再逐字符渲染 */
    uint16_t cx = x;
    while (*str) {
        if (*str < 0x80) {
            if (cx + plcd->font.width <= plcd->hw->width)
                lcd_show_char(plcd, cx, y, *str);
            cx += plcd->font.width;
            str++;
        } else {
            if (!*(str + 1)) break;
            uint8_t first  = *str;
            uint8_t second = *(str + 1);
            if (cx + 16 <= plcd->hw->width) {
                if (first >= 0xA1 && first <= 0xF7
                    && second >= 0xA1 && second <= 0xFE && ch_idx < 16) {
                    lcd_show_gbk_char_data(plcd, cx, y,
                                           font_buf + ch_idx * FONT_CHAR_SIZE);
                    ch_idx++;
                } else {
                    /* 超出 GB2312 范围, 画框占位 */
                    lcd_draw_rectangle(plcd, cx, y, cx + 15, y + 15, WHITE);
                }
            }
            cx += 16;
            str += 2;
